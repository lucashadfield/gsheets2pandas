...
class GSheetReader:
    ...
    def _sheet_data_to_dataframe(self, data: list, header=True) -> DataFrame:
        ...
        return (
            ...
            if header
            else DataFrame(data_list)
        )

    def fetch_spreadsheet(
        self, spreadsheet_id: str, sheet_name: Optional[str] = None, header: bool = True
    ) -> List[DataFrame]:
        spreadsheet_data = (
            self.service.spreadsheets()
            .get(spreadsheetId=spreadsheet_id, ranges=sheet_name, fields=FIELDS)
            .execute()
        )
