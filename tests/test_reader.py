...
def fetch_spreadsheet_side_effect(spreadsheetId, ranges=None, fields=None):
    ...
    if ranges is None:
        ...
        sheet_id_map = {
            s["properties"]["title"]: i
            for i, s in enumerate(sheet_info[spreadsheetId]["sheets"])
        }
        data[spreadsheetId]["sheets"] = [
            data[spreadsheetId]["sheets"][sheet_id_map[ranges]]
        ]
...
def mocked_reader(mock_discovery, mock_file):
    ...
    reader = GSheetReader()
    reader.service.spreadsheets.return_value.get.side_effect = (
        fetch_spreadsheet_side_effect
    )
